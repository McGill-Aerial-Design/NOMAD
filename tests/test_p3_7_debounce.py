...
def test_debounce_behavior(jetson_ip, duration_seconds=60, debug=False):
    ...
    print(f"\n{'='*70}")
    print(f"P3-7 Mesh Rebuild Debounce Test")
    print(f"{'='*70}")
    print(f"Target: {uri}")
    print(f"Duration: {duration_seconds}s")
    print(f"Expected: No more than 1 rebuild per 250ms window")
    ...
    try:
        ...
        if stats['inter_mesh_times']:
            ...
            if too_fast / len(inter_times) > 0.2:
                ...
                print(f"  Expected: <50ms (at 30Hz frame rate)")
                if debug:
        ...
        if stats['mesh_frames'] == 0:
            print(f"⚠ No mesh updates received (mesh data not present)")
            print(f"  This is OK if no mesh data is being streamed")
            print(f"  Recommendation: Run test while mesh is actively updating")
        ...
        if stats['inter_mesh_times']:
            ...
            if too_fast / len(stats['inter_mesh_times']) <= 0.2:
                print(f"✓ PASS: Debounce behavior verified")
                ...
                print(f"✗ FAIL: Too many rapid updates detected")
                print(f"  {too_fast}/{len(stats['inter_mesh_times'])} intervals < 250ms")
                print(f"  Debounce may not be functioning correctly")
                print(f"{'='*70}\n")
                return False
...
def test_mesh_visual_quality(jetson_ip, duration_seconds=30):
    ...
    print(f"\n{'='*70}")
    print(f"P3-7 Mesh Visual Quality Check")
    print(f"{'='*70}\n")
        ...
        while time.time() - start < duration_seconds and len(mesh_samples) < 5:
            try:
                ...
                if frame.get('type') == 'mesh' and 'mesh' in frame:
                    ...
                    print(f"✓ Mesh sample {len(mesh_samples)}: "
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Test P3-7 Mesh Rebuild Debounce functionality'
    )
    parser.add_argument(
        '--jetson-ip',
    ...
    parser.add_argument(
        '--visual-check',
        action='store_true',
        help='Also check mesh visual quality'
    )
    parser.add_argument(
        '--debug',
        action='store_true',
