...
Building the plugin is **Windows-only** — it links against Mission Planner's
own DLLs, so CI cannot build it. To run `pixi run build-plugin` you need:

1. **Windows** with **MSBuild** — from Visual Studio 2022 (Community is free) or
   the standalone Build Tools for Visual Studio.
2. **A Mission Planner installation** — the build references `MissionPlanner.exe`,
   `MissionPlanner.Comms.dll`, `MAVLink.dll`, etc. from it. Download from
   [ardupilot.org](https://ardupilot.org/planner/docs/mission-planner-installation.html).
3. **.NET Framework 4.8** developer pack (the project targets `v4.8`).
4. **(Optional) LibVLCSharp + VLC** — for enhanced embedded video playback.
...
> **Disposable build artifacts.** `src/bin/` (Debug/Release output) and
> `packaging/libvlc-windows/` (fetched VLC redistributables) are gitignored and
> safe to delete at any time — they are regenerated by `build-plugin` and
> `packaging/fetch-libvlc.ps1`. `copy-libvlc.ps1` copies from the single
> `packaging/libvlc-windows/` source into `bin/Release`, so there is one
> authoritative copy of the redistributables to maintain. If `mission_planner/`
> grows large on disk, deleting `bin/` and `libvlc-windows/` reclaims it.
...
The dual-link sender can carry custom `COMMAND_LONG` IDs over the ELRS link for
mission-specific actions. The baseline ships the transport but no mission
commands; deployment-specific command IDs (e.g. the AEAC2026 capture/reset
commands) live on the `AEAC2026` branch alongside their views.
