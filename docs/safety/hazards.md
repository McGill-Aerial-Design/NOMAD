...
The last open item — **H-05 SITL evidence** — was closed on 2026-06-11: the
containment scenario (`pixi run sitl-fence`) passed against live ArduPilot SITL
at commit f69a137 (in-fence target accepted, out-of-fence GLOBAL_INT and
LOCAL_NED targets rejected, vehicle contained). The velocity loop-closure
scenario (`pixi run sitl-scenario`) was re-run the same day at the same commit
and passed (peak 1.49 m/s under clamp, watchdog stop to 0.05 m/s, LOITER
setpoints refused). No hazard row carries a **GAP** marker.
