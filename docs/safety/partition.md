...
| Payload arm/release command logic (currently inside `PayloadControlPanel`) | The release command must go through a validated SC call with explicit confirm + interlock. Rearchitecture §3.3. **GAP — release logic still mixed with panel chrome.** |
