...
def test_ros_topic():
    ...
    try:
        ...
        node.create_subscription(Marker, "/nvblox_node/color_layer_marker", callback, 10)
...
_DEFAULT_BASE_URL = os.environ.get("NOMAD_API_URL", "http://172.17.0.1:8000")


def test_edge_core_api(base_url=_DEFAULT_BASE_URL):
    """Check if Edge Core is receiving mesh data."""
    print("\n" + "=" * 70)
    print("TEST 2: Edge Core API /api/slam/mesh")
    print("=" * 70)

    try:
        url = f"{base_url}/api/slam/mesh?format=summary"
        print(f"  GET {url}")
        ...
        if data.get("available"):
            print("  ✓ PASSED: Mesh data available")
...
def test_ros_http_bridge(base_url=_DEFAULT_BASE_URL):
...
def test_websocket(base_url=_DEFAULT_BASE_URL):
        ...
        while time.time() - start < 5.0:
            try:
                ...
                if msg_type == "pose":
                    ...
                    if pose_count <= 3:
                        print(
                            f"    - Pose #{pose_count}: x={data.get('x', 0):.2f}, y={data.get('y', 0):.2f}, yaw={data.get('body_yaw', 0):.1f}°"
                        )
                elif msg_type == "mesh":
                    mesh_received = True
...
def main():
    ...
    parser.add_argument("--base-url", default=_DEFAULT_BASE_URL, help="Edge Core base URL")
