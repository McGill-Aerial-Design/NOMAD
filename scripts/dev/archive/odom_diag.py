...
class OdomDiagNode(Node):
    def __init__(self):
        ...
        self.sub = self.create_subscription(Odometry, "/zed/zed_node/odom", self.callback, 10)
        self.count = 0
    ...
    def callback(self, msg):
        ...
        if self.count % 30 == 0:  # Print every ~1 second at 30Hz
            ...
            print(
                f"Euler: roll={math.degrees(roll):.1f}° pitch={math.degrees(pitch):.1f}° yaw={math.degrees(yaw):.1f}°"
            )
            print("---")
        self.count += 1
