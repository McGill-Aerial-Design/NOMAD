...
3. **Building model** determines which face the target is on and its position relative to the nearest corner
...
0.9m from the southwest corner.
...
├── target_localizer/
│   ├── __init__.py
│   ├── building_model.py       # Building geometry, faces, corners, projections
│   ├── detectors.py            # HSV circle detector
...
- Descriptions reference the building corners (NW/NE/SW/SE) reconstructed
  from the four GPS corner captures — pure GPS-based positioning.
