...
COMMAND_WHITELIST: dict[str, str] = {
    ...
    "start_novnc": "if ss -ltn | grep -q ':6080 '; then echo 'already running'; else if ! ss -ltn | grep -q ':5901 '; then vncserver :1 -localhost yes -geometry ${NOVNC_GEOMETRY:-1280x720} >/dev/null 2>&1; sleep 2; fi; if ! ss -ltn | grep -q ':5901 '; then echo failed; exit 1; fi; nohup websockify --web /usr/share/novnc/ 6080 localhost:5901 > ~/nomad_logs/novnc.log 2>&1 & sleep 1; ss -ltn | grep -q ':6080 ' && echo started || (echo failed; exit 1); fi",
    "stop_novnc": "stopped=0; pkill -f '[w]ebsockify.*6080' 2>/dev/null && stopped=1; vncserver -kill :1 >/dev/null 2>&1 && stopped=1; pkill -f '[x]11vnc.*-rfbport 5900' 2>/dev/null && stopped=1; [ $stopped -eq 1 ] && echo stopped || echo 'not running'",
